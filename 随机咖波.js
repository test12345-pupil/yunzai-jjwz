...
export class random_capoos extends plugin {
    constructor(e) {
      ...
      this.random_image_data = random_image_data
    }
  
    async randomImage(){
      const ImageSetName = this.e.msg.replace(/^(#)?随机/, "");
      console.log(ImageSetName);
      if(this.random_image_data[ImageSetName]){
        let len = this.random_image_data[ImageSetName].length;
        const message = [
          segment.image(`${random_image_path}${ImageSetName}/${this.random_image_data[ImageSetName][Math.floor(Math.random()*len)]}`)
        ]
